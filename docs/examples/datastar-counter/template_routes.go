...
func TemplateRoutes(mux *http.ServeMux, receiver RoutesReceiver) TemplateRoutePaths {
	...
	mux.HandleFunc("POST /decrement", func(response http.ResponseWriter, request *http.Request) {
		defer func() {
		...
		flusher.Flush()
		var mut sync.Mutex
		receiver.Decrement(ctx, func(result int64) error {
			...
			buf.Reset()
			defer bytesBufferPool.Put(buf)
			td := SSETemplateData[RoutesReceiver, int64]{receiver: receiver, request: request, pathsPrefix: pathsPrefix, result: result}
			if err := templates.ExecuteTemplate(buf, "sseCount", &td); err != nil {
				slog.ErrorContext(request.Context(), "failed to render page", slog.String("path", request.URL.Path), slog.String("pattern", request.Pattern), slog.String("error", err.Error()))
				return err
			}
			td.data = buf
