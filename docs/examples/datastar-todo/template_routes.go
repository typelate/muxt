...
type RoutesReceiver interface {
	List(_ context.Context) (Todos, error)
	CreateTodo(_ context.Context, form TodoForm, sseTodos func(Todos) error)
	DeleteTodo(_ context.Context, id int, sseTodos func(Todos) error)
	ToggleTodo(_ context.Context, id int, sseTodos func(Todos) error)
}
...
func TemplateRoutes(mux *http.ServeMux, receiver RoutesReceiver) TemplateRoutePaths {
	...
	mux.HandleFunc("GET /", func(response http.ResponseWriter, request *http.Request) {
		var td = TemplateData[RoutesReceiver, Todos]{receiver: receiver, response: response, request: request, pathsPrefix: pathsPrefix}
		ctx := request.Context()
		...
		if len(td.errList) == 0 {
			var err error
			td.result, err = receiver.List(ctx)
			if err != nil {
	...
	mux.HandleFunc("POST /todos", func(response http.ResponseWriter, request *http.Request) {
		...
		flusher.Flush()
		var mut sync.Mutex
		receiver.CreateTodo(ctx, form, func(result Todos) error {
			...
			buf.Reset()
			defer bytesBufferPool.Put(buf)
			td := SSETemplateData[RoutesReceiver, Todos]{receiver: receiver, request: request, pathsPrefix: pathsPrefix, result: result}
			if err := templates.ExecuteTemplate(buf, "sseTodos", &td); err != nil {
				slog.ErrorContext(request.Context(), "failed to render page", slog.String("path", request.URL.Path), slog.String("pattern", request.Pattern), slog.String("error", err.Error()))
				return err
			}
			td.data = buf
	...
	mux.HandleFunc("POST /todos/{id}/toggle", func(response http.ResponseWriter, request *http.Request) {
		defer func() {
			_ = request.Body.Close()
		}()
		flusher, ok := response.(http.Flusher)
