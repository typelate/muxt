...
| `--wire-typelate-sse` | bool | `false` | Wire SSE routes through [github.com/typelate/sse](https://github.com/typelate/sse) instead of generating the wire protocol: `sse.New` owns the stream headers, flushing, and goroutine safety, and the `SSETemplateData` setters map to `MessageOption` values. Callback parameters may append a variadic `...sse.MessageOption` that the generated closure forwards. The module must be in the project's go.mod. |
