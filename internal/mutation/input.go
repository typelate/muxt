...
// traversal, enumeration, the --diff comparison -- reads this and the
// template files its definitions name, and needs no go command, so a test
// can plan from a package built in memory.
type input struct {
	dir string
	pkg source.Package
}
