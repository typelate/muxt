...
func TestArgument(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		Template string
		Expect   func(t *testing.T, defs []muxt.Definition, err error)
	}{
		{Name: "no args", Receiver: serverType, Template: `{{define "GET / M()"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
		...
		{Name: "package function call", Receiver: serverType, Template: `{{define "GET / FunctionContext(ctx)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Len(t, defs, 1)
		...
		{Name: "multipart raw pointer", Receiver: serverType, Template: `{{define "GET / MultipartFormPtr(multipart)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Len(t, defs, 1)
		...
		{Name: "form struct", Receiver: serverType, Template: `{{define "GET / FormStruct(form)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Len(t, defs, 1)
		...
		{Name: "path value", Receiver: serverType, Template: `{{define "GET /{id} String(id)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Len(t, defs, 1)
		...
		{Name: "body on a synthesized method is the reader type", Receiver: emptyStruct, Template: `{{define "POST / SaveBody(body)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Len(t, defs, 1)
			require.Equal(t, muxt.ArgumentTypeRequestBody, defs[0].Arguments[0].Type)
			require.True(t, types.Identical(ioReaderType, defs[0].Arguments[0].ParamType))
		...
		{Name: "marshalJSON with data result", Receiver: serverType, Template: `{{define "GET / marshalJSON(M())"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Equal(t, muxt.RepresentationMarshalJSON, defs[0].Representation)
			require.Equal(t, muxt.ResultShapeData, defs[0].ResultShape())
		}},
		{Name: "marshalJSON with data and error results", Receiver: serverType, Template: `{{define "GET / marshalJSON(StringError())"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Equal(t, muxt.RepresentationMarshalJSON, defs[0].Representation)
			require.Equal(t, muxt.ResultShapeDataError, defs[0].ResultShape())
		}},
		{Name: "marshalJSON requires a result", Receiver: serverType, Template: `{{define "GET / marshalJSON(NoResults())"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "marshalJSON requires a result to marshal but NoResults() returns nothing")
		}},
		{Name: "marshalJSON requires a non-error result", Receiver: serverType, Template: `{{define "GET / marshalJSON(NoParams())"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "marshalJSON requires a non-error result but NoParams() error only returns an error")
		}},
		{Name: "marshalJSON second result must be an error", Receiver: serverType, Template: `{{define "GET / marshalJSON(StringOK())"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "marshalJSON requires the second result of StringOK() (string, bool) to be an error, got bool")
		}},
		{Name: "marshalJSON first result must not be an error", Receiver: serverType, Template: `{{define "GET / marshalJSON(TwoErrors())"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
		...
		{Name: "marshalJSON allows at most two results", Receiver: serverType, Template: `{{define "GET / marshalJSON(ThreeResults())"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "marshalJSON allows at most two results but ThreeResults() (int, int, error) has 3")
		}},
		{Name: "signals callback with exact error result", Receiver: serverType, Template: `{{define "GET /b sse(StreamGoodSignals(countsSignals))"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Equal(t, muxt.ArgumentTypeSignalsCallback, defs[0].Arguments[0].Type)
		}},
		{Name: "signals callback result must be exactly error", Receiver: serverType, Template: `{{define "GET /b sse(StreamBadSignals(countsSignals))"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "the countsSignals signals callback must be a func(T) error")
		}},
		{Name: "nested method call", Receiver: serverType, Template: `{{define "GET / Any(Context(ctx))"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Len(t, defs, 1)
		...
		{Name: "nested package function call", Receiver: serverType, Template: `{{define "GET / Any(FunctionContext(ctx))"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			...
			requireArgument(t, nested.Arguments(), 0, "ctx", muxt.ArgumentTypeRequestContext, "example.com.Context")
		}},
		{Name: "synthesized method", Receiver: emptyStruct, Template: `{{define "GET / DoesNotExist(ctx)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Len(t, defs, 1)
		...
		{Name: "passing context argument when parameter is a string", Receiver: serverType, Template: `{{define "GET / String(ctx)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method expects type string but ctx is Context")
		}},
		{Name: "passing request when parameter is a receiver pointer", Receiver: serverType, Template: `{{define "GET / PtrServer(request)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method expects type *Server but request is *Request")
		}},
		{Name: "passing response when parameter is a string", Receiver: serverType, Template: `{{define "GET / String(response)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method expects type string but response is ResponseWriter")
		}},
		{Name: "too few arguments", Receiver: serverType, Template: `{{define "GET / String()"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, `handler func String(string) any expects 1 arguments but call String() has 0`)
			require.Len(t, defs, 1)
		}},
		{Name: "too many arguments", Receiver: serverType, Template: `{{define "GET /{name} Context(ctx, name)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, `handler func Context(Context) any expects 1 arguments but call Context(ctx, name) has 2`)
		}},
		{Name: "execute callback when method takes no parameters", Receiver: serverType, Template: `{{define "GET / NoParams(execute)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "execute argument for NoParams must be a func(...) error")
		}},
		{Name: "wrong argument type in shared field list", Receiver: serverType, Template: `{{define "GET /post/{postID}/comment/{commentID} FieldList(ctx, request, commentID)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method expects type string but request is *Request")
		}},
		{Name: "data result shape", Receiver: serverType, Template: `{{define "GET / M()"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Equal(t, muxt.ResultShapeData, defs[0].ResultShape())
		}},
		{Name: "data and error result shape", Receiver: serverType, Template: `{{define "GET / StringError()"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Equal(t, muxt.ResultShapeDataError, defs[0].ResultShape())
		}},
		{Name: "data and ok result shape", Receiver: serverType, Template: `{{define "GET / StringOK()"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Equal(t, muxt.ResultShapeDataOK, defs[0].ResultShape())
		}},
		{Name: "method with no results", Receiver: serverType, Template: `{{define "GET / NoResults()"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method NoResults() has no results; it should have one or two")
		}},
		{Name: "second result must be error or bool", Receiver: serverType, Template: `{{define "GET / TwoResultsSecondNotErrorOrBool()"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
		...
		{Name: "execute method must return only error", Receiver: serverType, Template: `{{define "GET / ExecuteReturnsValue(execute)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method ExecuteReturnsValue(func() error) (int, error) receiving the execute callback must return only error")
		}},
		{Name: "sse method must return nothing or an error", Receiver: serverType, Template: `{{define "GET /x sse(SSEReturnsValue(execute))"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "sse handler method SSEReturnsValue(func(string) error) int must return nothing or a single error")
		}},
		{Name: "sse method returning nothing", Receiver: serverType, Template: `{{define "GET /x sse(SSEEvents(execute))"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Equal(t, muxt.ResultShapeNone, defs[0].ResultShape())
		}},
		...
		{Name: "execute callback with too many parameters", Receiver: serverType, Template: `{{define "GET / ExecuteMultiArg(execute)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "execute callback must have zero or one parameter; wrap multiple values in a struct")
		}},
		{Name: "sse callback parameter is not a function", Receiver: serverType, Template: `{{define "GET /x sse(SSECallbackNotFunc(execute))"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "execute parameter for SSECallbackNotFunc must be a function")
		}},
		{Name: "sse callback with too many parameters", Receiver: serverType, Template: `{{define "GET /x sse(SSECallbackMultiArg(execute))"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "sse callback must have zero or one parameter; wrap multiple values in a struct")
		}},
		{Name: "execute callback method not defined on receiver", Receiver: emptyStruct, Template: `{{define "GET / NotDefined(execute)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method NotDefined using the execute callback must be defined on the receiver type")
		}},
		{Name: "sse prefixed callback template missing", Receiver: serverType, Template: `{{define "GET /events sse(SSETwoCallbacks(execute, sseClock))"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, `no template "sseClock" for sse argument sseClock`)
		}},
		{Name: "sse prefixed callback template defined", Receiver: serverType, Template: `{{define "GET /events sse(SSETwoCallbacks(execute, sseClock))"}}{{end}}{{define "sseClock"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
		...
		{Name: "sse message template missing", Receiver: serverType, Template: `{{define "GET /x sse(SSECallbackNotFunc(fooMessage))"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, `no template "fooMessage" for sse message argument fooMessage`)
		}},
		{Name: "sse message template defined", Receiver: serverType, Template: `{{define "GET /x sse(SSECallbackNotFunc(fooMessage))"}}{{end}}{{define "fooMessage"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Equal(t, muxt.ArgumentTypeSendMessage, defs[0].Arguments[0].Type)
			require.NotNil(t, defs[0].Arguments[0].Template())
			require.Equal(t, "fooMessage", defs[0].Arguments[0].Template().Name())
		}},
		{Name: "method with three results", Receiver: serverType, Template: `{{define "GET / ThreeResults()"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method ThreeResults() (int, int, error) has 3 results; it should have one or two")
		}},
		{Name: "nested call with three results", Receiver: serverType, Template: `{{define "GET / Any(ThreeResults())"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method ThreeResults() (int, int, error) has 3 results; it should have one or two")
		}},
		{Name: "path value with unsupported basic type", Receiver: serverType, Template: `{{define "GET /{id} Float64(id)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method param type float64 not supported")
		}},
		{Name: "path value with unsupported named type", Receiver: serverType, Template: `{{define "GET /{id} URLParam(id)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "unsupported type: URL")
		}},
		{Name: "path value with text unmarshaler", Receiver: serverType, Template: `{{define "GET /{id} TextUnmarshalerParam(id)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Equal(t, muxt.ArgumentTypeRequestPathValue, defs[0].Arguments[0].Type)
		}},
		{Name: "last event id with unsupported basic type", Receiver: serverType, Template: `{{define "GET / Float64(lastEventID)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "method param type float64 not supported")
		}},
		{Name: "form struct with unsupported field type", Receiver: serverType, Template: `{{define "GET / FormUnsupportedField(form)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "failed to generate parse statements for form field href: unsupported type: URL")
		}},
		{Name: "multipart struct with file header fields", Receiver: serverType, Template: `{{define "POST / Upload(multipart)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.Equal(t, muxt.ArgumentTypeRequestMultipartForm, defs[0].Arguments[0].Type)
		}},
		{Name: "multipart struct with unsupported field type", Receiver: serverType, Template: `{{define "POST / BadUpload(multipart)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "failed to generate parse statements for multipart field File: unsupported type: File")
		}},
		{Name: "form struct with file header field is unsupported", Receiver: serverType, Template: `{{define "GET / Upload(form)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.ErrorContains(t, err, "failed to generate parse statements for form field File: unsupported type: *FileHeader")
		}},
		{Name: "form struct field bindings", Receiver: serverType, Template: `{{define "GET / TaggedForm(form)"}}{{end}}{{define "count-template"}}<input name="count-input" minlength="1">{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			...
			require.Equal(t, muxt.UnmarshalInt, count.Method)
			require.Len(t, count.Validations, 1)
			minLength, ok := count.Validations[0].(muxt.MinLengthValidation)
			require.True(t, ok)
		...
		{Name: "multipart struct field bindings", Receiver: serverType, Template: `{{define "POST / Upload(multipart)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			fields := defs[0].Arguments[0].FormFields()
		...
		{Name: "raw form param has no field bindings", Receiver: serverType, Template: `{{define "GET / URLValues(form)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
		...
		// first. A value the method takes as it arrives is direct.
		{Name: "a request value its parameter takes as it is, is direct", Receiver: serverType, Template: `{{define "GET / Context(ctx)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.True(t, defs[0].Arguments[0].Direct(), "ctx passes to a Context parameter as it is")
		}},
		{Name: "a form its parameter takes as it is, is direct", Receiver: serverType, Template: `{{define "GET / URLValues(form)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.True(t, defs[0].Arguments[0].Direct(), "form passes to a Values parameter as it is")
		}},
		{Name: "a form bound to a struct is not direct", Receiver: serverType, Template: `{{define "GET / FormStruct(form)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.False(t, defs[0].Arguments[0].Direct(), "a struct parameter is bound field by field, not assigned")
			require.NotEmpty(t, defs[0].Arguments[0].FormFields())
		}},
		{Name: "a body its parameter takes as it is, is direct", Receiver: serverType, Template: `{{define "POST / Reader(body)"}}{{end}}`, Expect: func(t *testing.T, defs []muxt.Definition, err error) {
			require.NoError(t, err)
			require.True(t, defs[0].Arguments[0].Direct(), "body passes to a Reader parameter as it is")
		}},
	} {
...
func requireArgument(t *testing.T, args []muxt.Argument, i int, identifier string, argType muxt.ArgumentType, paramType string) {
	t.Helper()
...
// its own stand-ins for the standard library types.
func exampleTypes(t *testing.T) *types.Package {
	t.Helper()
	files := make(map[string]string)
	for _, name := range []string{"functions.go", "methods.go", "std.go"} {
